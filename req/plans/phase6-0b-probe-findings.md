...
The probe uses logits `[[1, 3, 2, 0], [5, 4, 3, 0]]` (distinct values per row) and selects axis 1.
...
- Greedy decoding retains the existing axis-aware `mlx_argmax_axis` path. Equal maxima select the
  first index along the axis, evidenced by the dedicated tie probe above. `MLXOps.argmaxAxis`
