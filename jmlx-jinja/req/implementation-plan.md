...
- The Gradle configuration cache is currently disabled repo-wide (`gradle.properties`,
  diffplug/spotless#2850); design as if it will be re-enabled once that's fixed upstream --
  `upstreamVerify` must resolve project paths during configuration rather than retain Gradle model
  objects in execution closures. This single-project
