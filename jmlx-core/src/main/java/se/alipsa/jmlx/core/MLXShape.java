...
/**
 * Shape-manipulating ops: {@code reshape}, {@code broadcastTo}, {@code squeeze}, {@code transpose}, {@code slice}.
 * Split out of {@link MLX} as pure motion in req/phase4-plan.md M0a -- see that class's javadoc for the index of every
 * sibling this facade was split into.
 */
public final class MLXShape {
  ...
  public static MLXArray reshape(MLXArray a, int[] shape) {
    ...
    if (targetSize != a.size()) {
      throw new IllegalArgumentException("reshape: shape " + Arrays.toString(shape) + " (size " + targetSize
    ...
    try (Arena tmp = Arena.ofConfined()) {
      ...
      NativeOps.checked("reshape",
          () -> mlx_h.mlx_reshape(res, a.handle(), nativeShape, shape.length, NativeOps.DEFAULT_STREAM));
   ...
   * <em>not</em> the symmetric predicate {@link MLXOps}'s broadcast-compatibility check uses --
   * {@code broadcastTo([3], [1])} satisfies the symmetric rule but native rejects it. Also rejects any negative element
   * of {@code targetShape} outright, independent of the shape arithmetic: {@code broadcastTo([1], [-1])} would
   * otherwise pass here, since {@code 1 == 1} makes the directional check succeed too, and the failure would surface as
   * {@link MLXException} from native instead of {@link IllegalArgumentException} from Java.
   */
  private static void requireBroadcastableTo(MLXArray a, int[] targetShape) {
    for (int t : targetShape) {
      if (t < 0) {
        throw new IllegalArgumentException(
            "broadcastTo: target shape " + Arrays.toString(targetShape) + " has a negative dimension");
      }
    }
    int[] sa = a.shape();
    if (sa.length > targetShape.length) {
      throw new IllegalArgumentException(
          "broadcastTo: cannot broadcast " + Arrays.toString(sa) + " to " + Arrays.toString(targetShape));
    }
  ...
  private static MLXArray sliceNative(MLXArray a, int[] start, int[] stop, int[] strides) {
    ...
    try (Arena tmp = Arena.ofConfined()) {
      ...
      MemorySegment res = mlx_h.mlx_array_new(scope);
      NativeOps.checked("slice", () -> mlx_h.mlx_slice(res, a.handle(), nativeStart, start.length, nativeStop,
          stop.length, nativeStrides, strides.length, NativeOps.DEFAULT_STREAM));
      return new MLXArray(scope, res);
    }
  }
}
