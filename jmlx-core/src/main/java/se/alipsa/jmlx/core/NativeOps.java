...
final class NativeOps {
   ...
   * struct-array shape both {@code mlx_vector_array_new_data} ({@link MLX#eval}'s vector
   ...
   * handle this method's caller will ever pass to {@code mlx_array_free}, so allocating it through a
   * scope would corrupt that scope's handle-tracking invariant.
   */
