...
unbound declarations from `unsupported-by-runtime` entries.
...
Add root `verifyMlxApiCallSites` in `buildSrc`, wired to root `check`. It scans handwritten
source and requires matching inventory membership for:
...
do not fail. Exclude generated and build output. Explicitly classify jextract implementation
...
avoidance, unknown uses, generated-source exclusion, named infrastructure exemptions, and
allowed/disallowed test exceptions. Classify all existing production/probe sites before enabling the
task and document the commands in `CLAUDE.md`.
...
`MLXOps.argmaxAxis` is the existing public path for the argmax probe; direct binding use is limited
to the missing-facade selection/random declarations below and is a documented inventory probe
exception.

Probe the exact pinned behavior of:

- `mlx_argmax_axis`: shape/dtype, axes, tie-breaking, and lazy evaluation;
...
- explicit random key construction and both `mlx_random_split` (two-way) and
  ...
  reproducibility, and independence of sibling keys. The `split_num` result decides whether
  per-request/batched streams are viable.
...
They use the shared bootstrap-pin parser. In native CI, provision Python MLX from the already
bootstrap-verified wheel under `native/scratch/wheel` when possible; otherwise use a lock file with
fully pinned frontend `mlx` and backend `mlx-metal` distributions and `--require-hashes`. Record the
frontend and backend versions separately, along with wheel SHA-256/provenance, mlx-c commit,
platform, and Python version. `verifyMlxOracle` verifies that exact environment and provenance;
...
pure-Java tokenizer goldens. The native job runs the header cross-check, installs/verifies the oracle
after bootstrap from verified/locked artifacts, then explicitly invokes root inventory/call-site
...
| `req/` | Generated inventory, mapping, probe findings, Tier-B manifest. |
...
generated with both pins and header/binding counts; the guard covers all four native-use forms; probe
findings resolve argmax and per-request `split_num` semantics (and correct the shipped 6.0 claim if
needed); oracle provenance and a deterministic fixture are committed; and credential-free Tier-A
decoder/tokenizer fixtures pass in their respective CI jobs.
