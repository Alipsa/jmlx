...
- [ ] Confirm JDK 21 or newer and the exact Node version in `upstream/upstream-lock.json`, then run `./gradlew :jmlx-jinja:releaseVerification` from a clean source checkout. Use `-PreleaseVerificationAllowDirty` only for development; its report is not a release candidate.
