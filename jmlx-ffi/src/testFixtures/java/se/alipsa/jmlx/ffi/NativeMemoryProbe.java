...
/**
 * Samples mlx-c's own active-memory counter ({@code mlx_get_active_memory}) -- the one way tests in
 * this project observe whether native memory was actually freed, rather than relying on GC/Cleaner
 * timing as a proxy. Lives here, in jmlx-ffi's testFixtures, rather than in jmlx-core: jmlx-core
 * tests and jmlx-ffi tests both need it, and jmlx-ffi is the lower module in that dependency
 * (jmlx-core depends on jmlx-ffi, not the reverse). Throws {@link IllegalStateException} rather
 * than jmlx-core's {@code MLXException} on failure for the same reason -- that type isn't visible
 * from here.
 */
    ...
    try (Arena tmp = Arena.ofConfined()) {
      ...
      if (status != 0) {
        ...
        throw new IllegalStateException(
            "mlx_get_active_memory failed with status "
                + status
