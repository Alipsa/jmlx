 ...
 *
 * <p>Runs ONLY via the {@code :jmlx-ffi:loaderGuardTest} Gradle task, which forks its own JVM with
 * {@code jmlx.library.path} pointed at a disposable copy of {@code native/install/lib} with {@code
 * mlx.metallib} removed -- see that task in {@code jmlx-ffi/build.gradle}. Excluded from the
 * default {@code test} task: {@code NativeLoader.ensureLoaded()} caches its outcome, so if this ran
 * in the same JVM as {@code NativeLoaderSmokeTest} (in either order), the result would depend on
 * which test happened to run first rather than on whether the metallib is actually present. This
 * class also never mutates the real staging directory -- it only ever reads whatever directory
 * {@code jmlx.library.path} points it at.
 */
class NativeLoaderMissingMetallibTest {
  ...
  void secondCallRethrowsTheCachedOriginalFailure() {
    IllegalStateException first =
        assertThrows(IllegalStateException.class, NativeLoader::ensureLoaded);
