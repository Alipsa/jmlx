...
final class JsOperations {
  ...
  static Value arithmetic(String operator, Value left, Value right) {
    ...
    return switch (operator) {
      ...
      case "-" -> numericValue(a - b, floating);
      case "*" -> numericValue(a * b, floating);
  ...
  static double toNumber(Value value) {
    if (value instanceof Value.IntegerValue x) {
      return x.value();
    }
    if (value instanceof Value.FloatValue x) {
      return x.value();
    }
    if (value instanceof Value.NullValue) {
      return 0;
    }
    if (value instanceof Value.BooleanValue x) {
      return x.value() ? 1 : 0;
    }
    if (value instanceof Value.StringValue x) {
      return stringNumber(x.value());
    }
    return Double.NaN;
  }
  ...
  static boolean looseEquals(Value left, Value right) {
    if (nilLike(left) || nilLike(right)) {
      return nilLike(left) && nilLike(right);
    }
    ...
    if (left instanceof Value.BooleanValue x) {
      return looseNumberEquals(x.value() ? 1 : 0, right);
    }
    if (right instanceof Value.BooleanValue x) {
      return looseNumberEquals(x.value() ? 1 : 0, left);
    }
  ...
  private static Value numericValue(double value, boolean floating) {
  ...
  private static Value numericAdd(Value left, Value right) {
    ...
    boolean floating = left instanceof Value.FloatValue || right instanceof Value.FloatValue;
    return numericValue(result, floating);
  }
  ...
  private static double stringNumber(String value) {
    ...
    if (text.matches("0[oO][0-7]+")) {
      return new BigInteger(text.substring(2), 8).doubleValue();
    }
    if (text.matches("0[bB][01]+")) {
      return new BigInteger(text.substring(2), 2).doubleValue();
    }
  ...
  static String trimEcmaWhitespace(String value) {
    ...
    while (start < end && isEcmaWhitespace(value.charAt(start))) {
      start++;
    }
    while (end > start && isEcmaWhitespace(value.charAt(end - 1))) {
      end--;
    }
  ...
  static String trimStartEcmaWhitespace(String value) {
    int start = 0;
    while (start < value.length() && isEcmaWhitespace(value.charAt(start))) {
      start++;
    }
  ...
  static String trimEndEcmaWhitespace(String value) {
    int end = value.length();
    while (end > 0 && isEcmaWhitespace(value.charAt(end - 1))) {
      end--;
    }
